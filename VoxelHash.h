...
class HashData {
	...
	__global__
	static void updatesdfframe(float3 pos, float3 normal);
    //static function for initialization
    __global__ 
    static void initializeHashEntry(HashEntry* d_hash, int hashNumBuckets, int hashBucketSize);

    __global__ 
    static void initializeHeap(unsigned int* d_heap, unsigned int numSDFBlocks);

    __global__ 
    static void initializeHeapCounter(unsigned int* d_heapCounter, unsigned int value);
	...
	// }
	__device__
	float computesdf(int3 voxelpos,float3 normal);
	...
	__device__ 
	int setVoxel(const int3& virtualVoxelPos, Voxel& voxelInput) const;
    ...
    __device__
	void insertHashEntryElement(const int3& pos);

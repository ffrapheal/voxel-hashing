...
__constant__ HashParams c_hashParams;//GPU 常量
...
struct HashData {
	...
	void allocate(const HashParams& params, bool dataOnGPU = true) {
		...
		if (m_bIsOnGPU) {
			...
			cudaMalloc(&d_hashDecision, sizeof(int)* params.m_hashNumBuckets * params.m_hashBucketSize);
			...
			cudaMalloc(&d_hashCompactifiedCounter, sizeof(int));
	...
	void free() {
		if (m_bIsOnGPU) {
			...
			cudaFree(d_hash);
			cudaFree(d_hashDecision);
			cudaFree(d_hashDecisionPrefix);
			cudaFree(d_hashCompactified);
			cudaFree(d_hashCompactifiedCounter);
	...
	HashData copyToCPU() const {
		...
		cudaMemcpy(hashData.d_hashDecision, d_hashDecision, sizeof(int)*params.m_hashNumBuckets * params.m_hashBucketSize, cudaMemcpyDeviceToHost);
		cudaMemcpy(hashData.d_hashDecisionPrefix, d_hashDecisionPrefix, sizeof(int)*params.m_hashNumBuckets * params.m_hashBucketSize, cudaMemcpyDeviceToHost);
		cudaMemcpy(hashData.d_hashCompactified, d_hashCompactified, sizeof(HashEntry)* params.m_hashNumBuckets * params.m_hashBucketSize, cudaMemcpyDeviceToHost);
		cudaMemcpy(hashData.d_hashCompactifiedCounter, d_hashCompactifiedCounter, sizeof(unsigned int), cudaMemcpyDeviceToHost);

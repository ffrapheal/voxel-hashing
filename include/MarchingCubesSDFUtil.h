...
//#endif
	...
	void extractIsoSurfaceAtPosition(const float3& worldPos, const HashData& hashData)
	{
		...
		float3 p000 = worldPos+make_float3(M, M, M); float dist000; uchar3 color000; bool valid000 = trilinearInterpolationSimpleFastFast(hashData, p000, dist000, color000);
		float3 p100 = worldPos+make_float3(P, M, M); float dist100; uchar3 color100; bool valid100 = trilinearInterpolationSimpleFastFast(hashData, p100, dist100, color100);
		float3 p010 = worldPos+make_float3(M, P, M); float dist010; uchar3 color010; bool valid010 = trilinearInterpolationSimpleFastFast(hashData, p010, dist010, color010);
		float3 p001 = worldPos+make_float3(M, M, P); float dist001; uchar3 color001; bool valid001 = trilinearInterpolationSimpleFastFast(hashData, p001, dist001, color001);
		float3 p110 = worldPos+make_float3(P, P, M); float dist110; uchar3 color110; bool valid110 = trilinearInterpolationSimpleFastFast(hashData, p110, dist110, color110);
		float3 p011 = worldPos+make_float3(M, P, P); float dist011; uchar3 color011; bool valid011 = trilinearInterpolationSimpleFastFast(hashData, p011, dist011, color011);
		float3 p101 = worldPos+make_float3(P, M, P); float dist101; uchar3 color101; bool valid101 = trilinearInterpolationSimpleFastFast(hashData, p101, dist101, color101);
		float3 p111 = worldPos+make_float3(P, P, P); float dist111; uchar3 color111; bool valid111 = trilinearInterpolationSimpleFastFast(hashData, p111, dist111, color111);
